...
class ContentSignIn(tk.CTkFrame):
    ...
    def initUI(self, master, width, height):
        ...
        # tk.CTkButton(self, command=lambda: (self.destroy(), ContentSignUp(self, width=330, height=470).place(x=0, y=0))).pack()
        # ! Сделать систему смены окон
...
class ContentSignUp(tk.CTkFrame):
    def __init__(self, master, width, height):
        super().__init__(master, width, height)
